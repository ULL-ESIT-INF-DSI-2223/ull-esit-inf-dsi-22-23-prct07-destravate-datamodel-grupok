...
describe("ColeccionUsuario", () => {
  ...
  it("should set the users from an array", () => {
    ...
    expect(coleccionUsuario.getUsuarios()).to.be.eql(
      coleccionUsuario2.getUsuarios()
    );
  });
  ...
  it("should modify the name of a user", () => {
    ...
    expect(
      coleccionUsuario.getUsuarios().get(usuario4.getID())!.getNombre()
    ).to.be.equal("usuario5");
  });
  ...
  it("should modify the password of a user", () => {
    ...
    expect(
      coleccionUsuario.getUsuarios().get(usuario4.getID())!.getContraseña()
    ).to.be.equal("2Pr*");
  });
  ...
  it("should modify the activity of a user", () => {
    ...
    expect(
      coleccionUsuario.getUsuarios().get(usuario4.getID())!.getActividades()
    ).to.be.equal(Actividad.running);
  });
  ...
  it("should add a friend to a user", () => {
    ...
    expect(
      coleccionUsuario.getUsuarios().get(usuario4.getID())!.getAmigosApp()
    ).to.be.eql([1]);
  });
  ...
  it("should add a favorite route to a user", () => {
    ...
    expect(
      coleccionUsuario.getUsuarios().get(usuario4.getID())!.getRutasFavoritas()
    ).to.be.eql([1]);
  });
  ...
  it("should add a route to the user's routes", () => {
    const usuario4 = new Usuario("usuario4", "1Pr*", Actividad.ciclismo);
    coleccionUsuario.insertar(usuario4);
    coleccionUsuario.addRutaRealizada(usuario4, {
      ruta: 1,
      fecha: "2020-12-12",
    });
    expect(
      coleccionUsuario.getUsuarios().get(usuario4.getID())!.getHistoricoRutas()
    ).to.be.eql([{ ruta: 1, fecha: "2020-12-12" }]);
  });
  ...
  it("should add a challenge to the user's challenges", () => {
    ...
    expect(
      coleccionUsuario.getUsuarios().get(usuario4.getID())!.getRetosActivos()
    ).to.be.eql([1]);
  });
  ...
  it("should throw an error when inserting a user with an existing name", () => {
    ...
    expect(() => coleccionUsuario.insertar(usuario5)).to.throw(
      "El usuario con nombre " + usuario5.getNombre() + " ya existe"
    );
  });
  ...
  it("should throw an error when inserting a user with an existing id", () => {
    const usuario4 = new Usuario("usuario4", "1Pr*", Actividad.ciclismo);
    coleccionUsuario.insertar(usuario4);
    expect(() => coleccionUsuario.insertar(usuario4)).to.throw(
      "El usuario con id " + usuario4.getID() + " ya existe"
    );
  });

  it("should throw an error when deleting a user that does not exist", () => {
    const usuario4 = new Usuario("usuario4", "1Pr*", Actividad.ciclismo);
    expect(() => coleccionUsuario.eliminar(usuario4)).to.throw(
      "El usuario que deseas eliminar no existe."
    );
  });
  ...
  it("should throw an error when modifying the name of a user with an existing name", () => {
    ...
    expect(() =>
      coleccionUsuario.modificarNombre(usuario4, "usuario5")
    ).to.throw("El usuario con nombre " + "usuario5" + " ya existe");
  });
});
