...
import {
  checkCoordenadas,
  stringToCoordenadas,
} from "./interfaces/coordenadasInterface";
...
export class Gestor {
          ...
          case "Unirse a un reto":
            ...
            inquirer
              .prompt({
                ...
                choices: Array.from(this.coleccionRetos.getRetos().values())
                  .filter(
                    (reto) =>
                      !reto.getUsuarios().includes(usuarioActual.getID())
                  )
                  .map((reto) => reto.getNombre()),
              })
              .then((respuesta2) => {
                // Cogemos el reto seleccionado
                const reto = Array.from(
                  this.coleccionRetos.getRetos().values()
                ).find((reto) => reto.getNombre() === respuesta2.reto);
                if (reto != undefined) {
                  ...
                  if (!reto.getUsuarios().includes(usuarioActual.getID())) {
                    ...
                    this.jsonColeccionUsuario.addRetosActivos(
                      usuarioActual,
                      reto.getID()
                    );
                    ...
                    this.jsonColeccionReto.addUsuario(
                      reto,
                      usuarioActual.getID()
                    );
                  }
            ...
            inquirer
              ...
              .then((respuesta2) => {
                ...
                if (reto != undefined) {
                  ...
                  this.jsonColeccionUsuario.eraseRetosActivos(
                    usuarioActual,
                    reto.getID()
                  );
                  ...
                  // Lo escribimos en el json
                  this.jsonColeccionReto.eraseUsuario(
                    reto,
                    usuarioActual.getID()
                  );
                } else {
            ...
            inquirer
              ...
              .then((respuesta2) => {
                ...
                if (usuarios.has(respuesta2.nombre)) {
                  ...
                  console.log(
                    `No se ha encontrado ningún usuario con el nombre ${respuesta2.nombre}.`
                  );
                  return this.volver(() => this.gestionUsuarios());
                }
                // Buscamos el amigo por su nombre dentro del map
                const nuevoAmigo = Array.from(usuarios.values()).find(
                  (usuario) => usuario.getNombre() === respuesta2.nombre
                );
                ...
                console.log(
                  `Amigo ${nuevoAmigo.getNombre()} añadido al usuario ${usuarioActual.getNombre()}.`
                );
                return this.volver(() => this.gestionUsuarios());
              });
            break;
          case "Borrar amigos":
            ...
            inquirer
              .prompt({
                type: "list",
                name: "nombre",
                choices: Array.from(usuarioActual.getAmigosApp().values())
                  .map((id) =>
                    this.coleccionUsuarios.getUsuario(id).getNombre()
                  )
                  .concat("Cancelar"),
              })
              .then((respuesta2) => {
                // Obtenemos el id del usuario que queremos borrar
                const idUsuarioBorrar = Array.from(
                  usuarioActual.getAmigosApp().values()
                ).find(
                  (id) =>
                    this.coleccionUsuarios.getUsuario(id).getNombre() ===
                    respuesta2.nombre
                ...
                // Lo escribimos en el fichero
                this.jsonColeccionUsuario.eraseAmigo(
                  usuarioActual,
                  idUsuarioBorrar
                );
                return this.volver(() => this.gestionUsuarios());
          ...
          case "Borrar":
            ...
            inquirer
              .prompt({
                ...
                choices: Array.from(this.coleccionGrupos.getGrupos().values())
                  .filter((grupo) => grupo.getCreador() === id)
                  .map((grupo) => grupo.getNombre())
                  .concat("Cancelar"),
              })
              .then((respuesta2) => {
                // Obtenemos el nombre del grupo que queremos borrar
                const grupoBorrar = Array.from(
                  this.coleccionGrupos.getGrupos().values()
                ).find((grupo) => grupo.getNombre() === respuesta2.nombre);

                // Comprobamos que el grupo exista
                if (grupoBorrar == undefined) {
                  throw new Error(
                    `No se ha encontrado ningún grupo con el nombre ${respuesta2.nombre}.`
                  );
                }
          ...
          case "Unirse":
            ...
            inquirer
              .prompt({
                ...
                choices: Array.from(this.coleccionGrupos.getGrupos().values())
                  .filter((grupo) => !new Set(grupo.getParticipantes()).has(id))
                  .map((grupo) => grupo.getNombre())
                  .concat("Cancelar"),
              })
              .then((respuesta2) => {
                // Obtenemos el nombre del grupo que queremos borrar
                const grupoUnirse = Array.from(
                  this.coleccionGrupos.getGrupos().values()
                ).find((grupo) => grupo.getNombre() === respuesta2.nombre);
  ...
  private modificarUsuario(): void {
    ...
    inquirer
      .prompt({
        ...
        choices: Array.from(usuarios.values())
      ...
      .then((respuesta) => {
        if (respuesta.usuario === "Cancelar") {
          ...
          const usuarioAModificar = Array.from(usuarios.values()).find(
            (usuario) => usuario.getNombre() === respuesta.usuario
          );
                ...
                switch (respuesta.opcion) {
                  ...
                  case "Modificar nombre de Usuario":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        try {
                          this.jsonColeccionUsuario.modificarNombre(
                            usuarioAModificar,
                            respuesta2.nombre
                          );
                          this.coleccionUsuarios.modificarNombre(
                            usuarioAModificar,
                            respuesta2.nombre
                          );
                          this.gestionInfo();
                  ...
                  case "Editar Actividad":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        this.gestionInfo();
                        this.jsonColeccionUsuario.modificarActividad(
                          usuarioAModificar,
                          respuesta2.actividad
                        );
                        this.coleccionUsuarios.modificarActividad(
                          usuarioAModificar,
                          respuesta2.actividad
                        );
                      });
                  ...
                  case "Añadir Amigo":
                    ...
                    inquirer
                      .prompt({
                        ...
                        name: "nombre",
                        message:
                          "Introduce el nombre del amigo que deseas añadir: ",
                      })
                      .then((respuesta2) => {
                        // Obtener el listado de usuarios
                        const usuarios = this.coleccionUsuarios.getUsuarios();
                        ...
                        console.log(
                          `Amigo ${nuevoAmigo.getNombre()} añadido al usuario ${usuarioAModificar.getNombre()}.`
                        );
                        return this.volver(() => this.modificarUsuario());
                      });
                    break;
                  case "Borrar Amigo":
                    ...
                    inquirer
                      .prompt({
                        ...
                        name: "nombre",
                        choices: Array.from(
                          usuarioAModificar.getAmigosApp().values()
                        )
                          .map((id) =>
                            this.coleccionUsuarios.getUsuario(id).getNombre()
                          )
                  ...
                  case "Añadir Rutas Favoritas":
                    ...
                    inquirer
                      .prompt({
                        ...
                        name: "nombre",
                        choices: Array.from(
                          this.coleccionRutas.getRutas().values()
                        )
                          .map((ruta) => ruta.getNombre())
                          .concat("Cancelar"),
                      })
                      .then((respuesta2) => {
                        if (respuesta2.usuario === "Cancelar") {
                        ...
                        const idRuta = Array.from(
                          this.coleccionRutas.getRutas().values()
                        )
                          .find(
                            (ruta) => ruta.getNombre() === respuesta2.nombre
                          )
                          ?.getID();
                        // Comprobamos que la ruta exista
                        if (idRuta == undefined) {
                          throw new Error(
                            `No se ha encontrado ninguna ruta con el nombre ${respuesta2.nombre}.`
                          );
                        }
                        // Añadimos la ruta a la lista de rutas favoritas del usuario actual
                        usuarioAModificar.addRutaFavorita(idRuta);
                        // Lo escribimos en el fichero
                        this.jsonColeccionUsuario.addRutaFavorita(
                          usuarioAModificar,
                          idRuta
                        );
                        return this.volver(() => this.modificarUsuario());
                  ...
                  case "Borrar Rutas Favoritas":
                    const rutasFavoritas = Array.from(
                      usuarioAModificar.getRutasFavoritas().values()
                    ).map((id) => this.coleccionRutas.getRuta(id).getNombre());
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        ...
                        const idRuta = Array.from(
                          ...
                          (id) =>
                            this.coleccionRutas.getRuta(id).getNombre() ===
                            respuesta2.nombre
                        );
                        // Comprobamos que la ruta exista
                        if (idRuta == undefined) {
                        ...
                        // Lo escribimos en el fichero
                        this.jsonColeccionUsuario.eraseRutaFavorita(
                          usuarioAModificar,
                          idRuta
                        );
                        return this.volver(() => this.modificarUsuario());
                  ...
                  case "Añadir Ruta Realizada":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        ...
                        usuarioAModificar.addRutaRealizada({
                          ruta: idRuta,
                          fecha:
                        ...
                        // Lo escribimos en el fichero
                        this.jsonColeccionUsuario.addRutaRealizada(
                          usuarioAModificar,
                          {
                            ruta: idRuta,
                            fecha:
                        ...
                        const rutaParticipada = Array.from(
                          this.coleccionRutas.getRutas().values()
                        ).find((ruta) => ruta.getID() === idRuta);
                        if (
                          rutaParticipada !== undefined &&
                          rutaParticipada
                            .getUsuariosVisitantes()
                            .find((id) => id === usuarioAModificar.getID()) ===
                            undefined
                        ) {
                          rutaParticipada.addUsuarioVisitante(
                            usuarioAModificar.getID()
                          );
                          this.jsonColeccionRuta.addUsuarioVisitante(
                            rutaParticipada,
                            usuarioAModificar.getID()
                          );
                        }
                  ...
                  case "Añadir Retos Activos":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        ...
                        const idReto = Array.from(
                          this.coleccionRetos.getRetos().values()
                        )
                          .find(
                            (reto) => reto.getNombre() === respuesta2.nombre
                          )
                          ?.getID();
                        // Comprobamos que el reto exista
                        if (idReto == undefined) {
                          throw new Error(
                            `No se ha encontrado ningún reto con el nombre ${respuesta2.nombre}.`
                          );
                        }
                  ...
                  case "Borrar Retos Activos":
                    const retosActivosUsuario = Array.from(
                      usuarioAModificar.getRetosActivos().values()
                    ).map((id) => this.coleccionRetos.getReto(id).getNombre());
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        ...
                        const idReto = Array.from(
                          ...
                          (id) =>
                            this.coleccionRetos.getReto(id).getNombre() ===
                            respuesta2.nombre
                        );
                        // Comprobamos que el reto exista
                        if (idReto == undefined) {
      ...
      .then((respuesta) => {
        ...
        switch (respuesta.ordenacion) {
          case "Alfabéticamente":
            array = [...this.coleccionUsuarios.getUsuarios().values()]
              .sort((a, b) => a.getNombre().localeCompare(b.getNombre()))
              .map((a) => ({ Nombre: a.getNombre() }));
          ...
          case "Cantidad de km (semanal)":
            array = [...this.coleccionUsuarios.getUsuarios().values()]
              .sort(
                (a, b) =>
                  a.getEstadisticas().semana.km - b.getEstadisticas().semana.km
              )
              .map((a) => ({
                Nombre: a.getNombre(),
                "Km(semana)": a.getEstadisticas().semana.km,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
          ...
          case "Cantidad de km (mensual)":
            array = [...this.coleccionUsuarios.getUsuarios().values()]
              .sort(
                (a, b) =>
                  a.getEstadisticas().mes.km - b.getEstadisticas().mes.km
              )
              .map((a) => ({
                Nombre: a.getNombre(),
                "Km(mes)": a.getEstadisticas().mes.km,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
          ...
          case "Cantidad de km (anual)":
            array = [...this.coleccionUsuarios.getUsuarios().values()]
              .sort(
                (a, b) =>
                  a.getEstadisticas().anio.km - b.getEstadisticas().anio.km
              )
              .map((a) => ({
                Nombre: a.getNombre(),
                "Km(año)": a.getEstadisticas().anio.km,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
    ...
    inquirer
      ...
      .then((respuesta) => {
        if (respuesta.usuario === "Cancelar") {
          ...
          const usuarioAEliminar = Array.from(usuarios.values()).find(
            (usuario) => usuario.getNombre() === respuesta.usuario
          );
          if (usuarioAEliminar) {
            ...
            console.log(
              `Usuario ${usuarioAEliminar.getNombre()} eliminado con éxito`
            );
          } else {
            console.log(`No se encontró el usuario ${respuesta.usuario}`);
          }
  ...
  private modificarGrupo(): void {
    ...
    inquirer
      .prompt({
        ...
        choices: Array.from(grupos.values())
          .map((grupo) => grupo.getNombre())
          .concat("Cancelar"),
      })
      .then((respuesta) => {
        if (respuesta.grupo === "Cancelar") {
          ...
          const grupoAModificar = Array.from(grupos.values()).find(
            (grupo) => grupo.getNombre() === respuesta.grupo
          );
          if (grupoAModificar) {
            console.clear();
            console.log("¿Qué atributo desea modificar?");
                ...
                switch (respuesta.opcion) {
                  ...
                  case "Modificar Nombre de Grupo":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        try {
                          this.jsonColeccionGrupo.modificarNombre(
                            grupoAModificar,
                            respuesta2.nombre
                          );
                          this.coleccionGrupos.modificarNombre(
                            grupoAModificar,
                            respuesta2.nombre
                          );
                          this.gestionGrupos();
                          ...
                          inquirer
                            .prompt({
                              ...
                              name: "volver",
                              message:
                                "Pulsa enter para volver a introducir un grupo",
                            })
                            .then(() => {
                              this.registrarGrupo();
                            });
                  ...
                  case "Añadir Participante":
                    ...
                    inquirer
                      .prompt({
                        ...
                        name: "nombre",
                        message:
                          "Introduce el nombre del participante que deseas añadir: ",
                      })
                      .then((respuesta2) => {
                        // Obtener el listado de usuarios
                        const usuarios = this.coleccionUsuarios.getUsuarios();
                        ...
                        const nuevoParticipante = Array.from(
                          usuarios.values()
                        ).find(
                          (usuario) => usuario.getNombre() === respuesta2.nombre
                        );
                        ...
                        grupoAModificar.addParticipante(
                          nuevoParticipante.getID()
                        );
                        // Lo escribimos en el fichero
                        this.jsonColeccionGrupo.addParticipante(
                          grupoAModificar,
                          nuevoParticipante.getID()
                        );

                        console.log(
                          `Participante ${nuevoParticipante.getNombre()} añadido al grupo ${grupoAModificar.getNombre()}.`
                        );
                        return this.volver(() => this.modificarGrupo());
                      });
                    break;
                  case "Borrar Participante":
                    ...
                    inquirer
                      .prompt({
                        ...
                        name: "nombre",
                        choices: Array.from(
                          grupoAModificar.getParticipantes().values()
                        )
                          .map((id) =>
                            this.coleccionUsuarios.getUsuario(id).getNombre()
                          )
                      ...
                      .then((respuesta2) => {
                        if (respuesta2.nombre === "Cancelar") {
                          ...
                          const idParticipanteBorrar = Array.from(
                            grupoAModificar.getParticipantes().values()
                          ).find(
                            (id) =>
                              this.coleccionUsuarios
                          ...
                          if (idParticipanteBorrar == undefined) {
                            throw new Error(
                              `No se ha encontrado ningún participante con el nombre ${respuesta2.nombre}.`
                            );
                          }
                          // Borramos el participante de la lista del grupo actual
                          grupoAModificar.eraseParticipante(
                            idParticipanteBorrar
                          );
                          // Lo escribimos en el fichero
                          this.jsonColeccionGrupo.eraseParticipante(
                            grupoAModificar,
                            idParticipanteBorrar
                          );
                          return this.volver(() => this.modificarGrupo());
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        if (respuesta2.nombre === "Cancelar") {
                          ...
                          // Lo escribimos en el fichero
                          this.jsonColeccionGrupo.addRutaFavorita(
                            grupoAModificar,
                            idRuta
                          );
                          return this.volver(() => this.modificarGrupo());
                  ...
                  case "Borrar Rutas Favoritas":
                    const rutasFavoritas = Array.from(
                      grupoAModificar.getRutasFavoritas().values()
                    ).map((id) => this.coleccionRutas.getRuta(id).getNombre());
                    if (rutasFavoritas.length === 0) {
                      console.log("No tienes rutas favoritas.");
                      return this.volver(() => this.modificarGrupo());
                      ...
                      inquirer
                        ...
                        .then((respuesta2) => {
                          if (respuesta2.nombre === "Cancelar") {
                            ...
                            // Lo escribimos en el fichero
                            this.jsonColeccionGrupo.eraseRutaFavorita(
                              grupoAModificar,
                              idRuta
                            );
                            return this.volver(() => this.modificarGrupo());
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        ...
                        if (respuesta2.nombre === "Cancelar") {
                          ...
                          // Añadimos la ruta a la lista de rutas realizadas del grupo actual
                          grupoAModificar.addRutaRealizada({
                            ruta: idRuta,
                            fecha:
                              new Date().toLocaleDateString() +
                              " " +
                          ...
                          // Lo escribimos en el fichero
                          this.jsonColeccionGrupo.addRutaRealizada(
                            grupoAModificar,
                            {
                              ruta: idRuta,
                              fecha:
                          ...
                          let participanteRuta = Array.from(
                            this.coleccionUsuarios.getUsuarios().values()
                          ).find(
                            (usuario) =>
                              usuario.getID() === grupoAModificar.getCreador()
                          );
                          let rutaParticipada = Array.from(
                            this.coleccionRutas.getRutas().values()
                          ).find((ruta) => ruta.getID() === idRuta);
                          if (participanteRuta !== undefined) {
                            participanteRuta.addRutaRealizada({
                              ruta: idRuta,
                              fecha:
                                new Date().toLocaleDateString() +
                                " " +
                          ...
                          if (
                            ...
                            rutaParticipada
                              .getUsuariosVisitantes()
                              .find(
                                (id) => id === grupoAModificar.getCreador()
                              ) === undefined
                          ) {
                            rutaParticipada.addUsuarioVisitante(
                              grupoAModificar.getCreador()
                            );
                            this.jsonColeccionRuta.addUsuarioVisitante(
                              rutaParticipada,
                              grupoAModificar.getCreador()
                          ...
                          for (const userID of grupoAModificar.participantes) {
                            participanteRuta = Array.from(
                              this.coleccionUsuarios.getUsuarios().values()
                            ).find((usuario) => usuario.getID() === userID);
                            rutaParticipada = Array.from(
                              this.coleccionRutas.getRutas().values()
                            ).find((ruta) => ruta.getID() === idRuta);
                            if (participanteRuta !== undefined) {
                              participanteRuta.addRutaRealizada({
            ...
            try {
              ...
              inquirer
                .prompt({
                  ...
                  name: "volver",
                  message:
                    "Pulsa enter para volver a introducir un nombre de grupo",
                })
                .then(() => {
                  this.registrarGrupo();
                });
  ...
  private listarGrupos(callback: (i: this) => void): void {
    ...
    inquirer
      ...
      .then((respuesta) => {
        ...
        switch (respuesta.ordenacion) {
          case "Alfabéticamente":
            array = [...this.coleccionGrupos.getGrupos().values()]
              .sort((a, b) => a.getNombre().localeCompare(b.getNombre()))
              .map((a) => ({ Nombre: a.getNombre() }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
          ...
          case "Cantidad de km (semanal)":
            array = [...this.coleccionGrupos.getGrupos().values()]
              .sort(
                (a, b) =>
                  a.getEstadisticasEntrenamiento().semana.km -
                  b.getEstadisticasEntrenamiento().semana.km
              )
              .map((a) => ({
                Nombre: a.getNombre(),
                "Km(semana)": a.getEstadisticasEntrenamiento().semana.km,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
          ...
          case "Cantidad de km (mensual)":
            array = [...this.coleccionGrupos.getGrupos().values()]
              .sort(
                (a, b) =>
                  a.getEstadisticasEntrenamiento().mes.km -
                  b.getEstadisticasEntrenamiento().mes.km
              )
              .map((a) => ({
                Nombre: a.getNombre(),
                "Km(mes)": a.getEstadisticasEntrenamiento().mes.km,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
          ...
          case "Cantidad de km (anual)":
            array = [...this.coleccionGrupos.getGrupos().values()]
              .sort(
                (a, b) =>
                  a.getEstadisticasEntrenamiento().anio.km -
                  b.getEstadisticasEntrenamiento().anio.km
              )
              .map((a) => ({
                Nombre: a.getNombre(),
                "Km(año)": a.getEstadisticasEntrenamiento().anio.km,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
          ...
          case "Número de miembros":
            array = [...this.coleccionGrupos.getGrupos().values()]
              .sort(
                (a, b) =>
                  a.getParticipantes().length - b.getParticipantes().length
              )
              .map((a) => ({
                Nombre: a.getNombre(),
                Miembros: a.getParticipantes().length + 1,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
    ...
    inquirer
      ...
      .then((respuesta) => {
        if (respuesta.grupo === "Cancelar") {
          ...
          const grupoAEliminar = Array.from(grupos.values()).find(
            (grupo) => grupo.getNombre() === respuesta.grupo
          );
          if (grupoAEliminar) {
            // Lo eliminamos del json
            ...
            console.log(
              `Grupo ${grupoAEliminar.getNombre()} eliminado con éxito`
            );
          } else {
            console.log(`No se encontró el usuario ${respuesta.usuario}`);
          }
  ...
  public registrarRuta(): void {
    console.clear();
    console.log(
      "Bienvenido a la creación de rutas. Por favor, rellene los siguientes campos:"
    );
    inquirer
      .prompt([
        {
      ...
      .then((respuesta: any) => {
        try {
          ...
          const coordenadasInicio = stringToCoordenadas(
  ...
  private listarRutas(callback: (i: this) => void): void {
    ...
    inquirer
      ...
      .then((respuesta) => {
        ...
        switch (respuesta.ordenacion) {
          case "Alfabéticamente":
            array = [...this.coleccionRutas.getRutas().values()]
              .sort((a, b) => a.getNombre().localeCompare(b.getNombre()))
              .map((a) => ({ Nombre: a.getNombre() }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
          ...
          case "Número de usuarios":
            array = [...this.coleccionRutas.getRutas().values()]
              .sort(
                (a, b) =>
                  a.getUsuariosVisitantes().length -
                  b.getUsuariosVisitantes().length
              )
              .map((a) => ({
                Nombre: a.getNombre(),
                "Usuarios de la ruta": a.getUsuariosVisitantes().length + 1,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
          ...
          case "Cantidad de km":
            array = [...this.coleccionRutas.getRutas().values()]
              .sort((a, b) => a.getLongitud() - b.getLongitud())
              .map((a) => ({ Nombre: a.getNombre(), Km: a.getLongitud() + 1 }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
            console.table(array);
          ...
          case "Calificación Media":
            array = [...this.coleccionRutas.getRutas().values()]
              .sort((a, b) => a.getCalificacion() - b.getCalificacion())
              .map((a) => ({
                Nombre: a.getNombre(),
                Miembros: a.getCalificacion(),
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
          ...
          case "Por actividad":
            array = [...this.coleccionRutas.getRutas().values()]
              .sort((a, b) =>
                a.getTipoActividad().localeCompare(b.getTipoActividad())
              )
              .map((a) => ({
                Actividad: a.getTipoActividad(),
                Nombre: a.getNombre(),
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
  ...
  private modificarRuta(): void {
    ...
    inquirer
      .prompt({
        ...
        choices: Array.from(rutas.values())
          .map((ruta) => ruta.getNombre())
          .concat("Cancelar"),
      })
      .then((respuesta) => {
        if (respuesta.ruta === "Cancelar") {
          ...
          const rutaAModificar = Array.from(rutas.values()).find(
            (ruta) => ruta.getNombre() === respuesta.ruta
          );
          if (rutaAModificar) {
            console.clear();
            console.log("¿Qué atributo desea modificar?");
                ...
                switch (respuesta.opcion) {
                  case "Modificar nombre de ruta":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        try {
                          this.jsonColeccionRuta.modificarNombreRuta(
                            rutaAModificar,
                            respuesta2.nombre
                          );
                          this.coleccionRutas.modificarNombreRuta(
                            rutaAModificar,
                            respuesta2.nombre
                          );
                          this.gestionRutas();
                          ...
                          inquirer
                            .prompt({
                              ...
                              name: "volver",
                              message:
                                "Pulsa enter para volver a introducir un ruta",
                            })
                            .then(() => {
                              this.registrarRuta();
                            });
                    ...
                    inquirer
                      ...
                      .then((respuesta: any) => {
                        try {
                          ...
                          const coordenadasFin = stringToCoordenadas(
                            respuesta.coordenadasFin
                          );

                          this.jsonColeccionRuta.modificarCoordenadasRuta(
                            rutaAModificar,
                            coordenadasInicio,
                            coordenadasFin
                          );
                          this.coleccionRutas.modificarCoordenadasRuta(
                            rutaAModificar,
                            coordenadasInicio,
                            coordenadasFin
                          );
                  ...
                  case "Modificar longitud":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        try {
                          this.jsonColeccionRuta.modificarLongitudRuta(
                            rutaAModificar,
                            respuesta2.longitud
                          );
                          this.coleccionRutas.modificarLongitudRuta(
                            rutaAModificar,
                            respuesta2.longitud
                          );
                          this.modificarRuta();
                  ...
                  case "Modificar desnivel":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        try {
                          this.jsonColeccionRuta.modificarDesnivelRuta(
                            rutaAModificar,
                            respuesta2.desnivel
                          );
                          this.coleccionRutas.modificarDesnivelRuta(
                            rutaAModificar,
                            respuesta2.desnivel
                          );
                          this.modificarRuta();
                  ...
                  case "Modificar tipo de actividad":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        try {
                          this.jsonColeccionRuta.modificarTipoActividadRuta(
                            rutaAModificar,
                            respuesta2.tipoActividad
                          );
                          this.coleccionRutas.modificarTipoActividadRuta(
                            rutaAModificar,
                            respuesta2.tipoActividad
                          );
                          this.modificarRuta();
                          ...
                          console.log(
                            "Introduce un tipo de actividad de ruta nuevo"
                          );
                          // pulsar enter para volver a introducir un nombre de Ruta
                          inquirer
                            .prompt({
                  ...
                  case "Modificar dificultad":
                    ...
                    inquirer
                      ...
                      .then((respuesta2) => {
                        try {
                          this.jsonColeccionRuta.modificarDificultadRuta(
                            rutaAModificar,
                            respuesta2.dificultad
                          );
                          this.coleccionRutas.modificarDificultadRuta(
                            rutaAModificar,
                            respuesta2.dificultad
                          );
                          this.modificarRuta();
    ...
    inquirer
      ...
      .then((respuesta) => {
        if (respuesta.ruta === "Cancelar") {
          ...
          const rutaAEliminar = Array.from(rutas.values()).find(
            (ruta) => ruta.getNombre() === respuesta.ruta
          );
          if (rutaAEliminar) {
            ...
            console.log(
              `ruta ${rutaAEliminar.getNombre()} eliminada con éxito`
            );
          } else {
    ...
    inquirer
      .prompt([
        ...
        },
        {
          type: "list",
          name: "ordenacion",
          choices: ["Alfabéticamente", "Cantidad de km", "Número de miembros"],
        },
      ])
      .then((respuesta) => {
        let array;
        switch (respuesta.ordenacion) {
          case "Alfabéticamente":
            array = [...this.coleccionRetos.getRetos().values()]
              .sort((a, b) => a.getNombre().localeCompare(b.getNombre()))
              .map((a) => ({ Nombre: a.getNombre() }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
          ...
          case "Cantidad de km":
            array = [...this.coleccionRetos.getRetos().values()]
              .sort((a, b) => a.getKmTotales() - b.getKmTotales())
              .map((a) => ({
                Nombre: a.getNombre(),
                "Km Totales": a.getKmTotales(),
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
          ...
          case "Número de miembros":
            array = [...this.coleccionRetos.getRetos().values()]
              .sort((a, b) => a.getUsuarios().length - b.getUsuarios().length)
              .map((a) => ({
                Nombre: a.getNombre(),
                Miembros: a.getUsuarios().length,
              }));
            if (respuesta.sentido === "Descendente") {
              array = array.reverse();
            }
  ...
  private modificarReto(): void {
    ...
    inquirer
      .prompt({
        ...
        choices: Array.from(retos.values())
          .map((reto) => reto.getNombre())
          .concat("Cancelar"),
      })
      .then((respuesta) => {
        if (respuesta.reto === "Cancelar") {
          ...
          // Buscar el grupo a modificar por su nombre y modificarlo
          const retoAModificar = Array.from(retos.values()).find(
            (reto) => reto.getNombre() === respuesta.reto
          );
          if (retoAModificar) {
            console.clear();
            console.log("¿Qué atributo desea modificar?");
              ...
              .then((respuesta) => {
                switch (respuesta.opcion) {
                    ...
                    inquirer
                      ...
                      .then((respuesta) => {
                        try {
                          ...
                          this.jsonColeccionReto.modificarNombre(
                            retoAModificar,
                            respuesta.nombre
                          );
                          console.log(
                            `Reto ${retoAModificar.getNombre()} modificado con éxito`
                          );
                    ...
                    inquirer
                      ...
                      .then((respuesta) => {
                        if (respuesta.ruta === "Cancelar") {
                          ...
                          const rutaAAñadir = Array.from(rutas.values()).find(
                            (ruta) => ruta.getNombre() === respuesta.ruta
                          );
                          if (rutaAAñadir) {
                            retoAModificar.addRuta(rutaAAñadir.getID());
                            this.jsonColeccionReto.addRuta(
                              retoAModificar,
                              rutaAAñadir.getID()
                            );
                            console.log(
                              `Ruta ${rutaAAñadir.getNombre()} añadida con éxito`
                            );
                            this.volver(() => this.modificarReto());
                          } else {
                            console.log(
                              `No se encontró la ruta ${respuesta.ruta}`
                            );
                            this.volver(() => this.modificarReto());
                          }
                        }
                  ...
                  case "Borrar Ruta":
                    ...
                    const rutaslist = Array.from(
                      retoAModificar.getRutas().values()
                    ).map((id) => this.coleccionRutas.getRuta(id).getNombre());
                    ...
                    inquirer
                      ...
                      .then((respuesta) => {
                        if (respuesta.ruta === "Cancelar") {
                          ...
                          if (rutaABorrar) {
                            this.jsonColeccionReto.eraseRuta(
                              retoAModificar,
                              rutaABorrar
                            );
                    ...
                    inquirer
                      ...
                      .then((respuesta) => {
                        if (respuesta.usuario === "Cancelar") {
                          ...
                          const usuarioAAñadir = Array.from(
                            usuarios.values()
                          ).find(
                            (usuario) =>
                              usuario.getNombre() === respuesta.usuario
                          );
                          if (usuarioAAñadir) {
                            retoAModificar.addUsuario(usuarioAAñadir.getID());
                            this.jsonColeccionReto.addUsuario(
                              retoAModificar,
                              usuarioAAñadir.getID()
                            );
                            console.log(
                              `Usuario ${usuarioAAñadir.getNombre()} añadido con éxito`
                            );
                            this.volver(() => this.modificarReto());
                          } else {
                            console.log(
                              `No se encontró el usuario ${respuesta.usuario}`
                            );
                            this.volver(() => this.modificarReto());
                          }
                        }
                  ...
                  case "Borrar Usuario de Reto":
                    console.clear();
                    // Obtener el listado de usuarios
                    const usuarioslist = Array.from(
                      retoAModificar.getUsuarios().values()
                    ).map((id) => this.coleccionUsuarios.getUsuario(id));
                    ...
                    inquirer
                      .prompt({
                        ...
                        choices: Array.from(usuarioslist.values())
                          .map((usuario) => usuario.getNombre())
                          .concat("Cancelar"),
                      })
                      .then((respuesta) => {
                        if (respuesta.usuario === "Cancelar") {
                          ...
                          const usuarioABorrar = Array.from(
                            usuarioslist.values()
                          ).find(
                            (usuario) =>
                              usuario.getNombre() === respuesta.usuario
                          ...
                          if (usuarioABorrar) {
                            retoAModificar.removeUsuario(
                              usuarioABorrar.getID()
                            );
                            this.jsonColeccionReto.eraseUsuario(
                              retoAModificar,
                              usuarioABorrar.getID()
                            ...
                            console.log(
                              `Usuario ${usuarioABorrar.getNombre()} borrado con éxito`
                            );
                            this.volver(() => this.modificarReto());
                          } else {
                            console.log(
                  ...
                  case "Modificar Actividad":
                    ...
                    inquirer
                      ...
                      .then((respuesta) => {
                        // set actividad
                        this.jsonColeccionReto.modificarActividad(
                          retoAModificar,
                          respuesta.actividad
                        );
                        this.coleccionRetos.modificarActividad(
                          retoAModificar,
                          respuesta.actividad
                        );
    ...
    inquirer
      ...
      .then((respuesta) => {
        if (respuesta.reto === "Cancelar") {
          ...
          const retoAEliminar = Array.from(retos.values()).find(
            (reto) => reto.getNombre() === respuesta.reto
          );
          if (retoAEliminar) {
            ...
            console.log(
              `Reto ${retoAEliminar.getNombre()} eliminado con éxito`
            );
          } else {
            console.log(`No se encontró el reto ${respuesta.reto}`);
          }
